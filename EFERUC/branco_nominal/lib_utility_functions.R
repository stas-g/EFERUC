...
read_dataset_names <- function(){
  read.table(file = "../with_nominal_datasets/datasets.txt", 
    header = F, stringsAsFactors = F)[, 1]
}

read_experiment_data <- function(dataset){
  df <- read.csv(
    file = paste0("../with_nominal_datasets/", dataset, ".csv"),
    header = TRUE
  )

  train_idx <- read.table(
    file = paste0("../with_nominal_datasets/splits/", dataset, "_train.txt"),
    header = FALSE,
    stringsAsFactors = FALSE)[,1]
  
  test_idx <- read.table(
    file = paste0("../with_nominal_datasets/splits/", dataset, "_test.txt"),
    header = FALSE,
    stringsAsFactors = FALSE)[,1]
