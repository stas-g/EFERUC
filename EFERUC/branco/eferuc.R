...
perform_feruc <- function(dataset_name, discretizer){
  ...
  for(response in colnames(expdata$ys_learning)){
    ...
    # Additional check, should never really happen but might for edge cases 
    # because of the < 50 condition in unsupervised_discretization.
    actual_max_bins <- length(layered_ylearning_bins)
    if (actual_max_bins != max_bins){
      message(paste0("  Changed max bins from ", max_bins, " to ", actual_max_bins))
      max_bins <- actual_max_bins
    }
    ...
    for (crpair_index in 1:nrow(CRPAIRS)) {
      ...
      names(y_test) <- rownames(expdata$ys_test)
