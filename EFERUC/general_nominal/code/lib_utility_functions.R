...
read_dataset_names <- function(){
  read.table(file = "../datasets/datasets.txt", header = F)[, 1]
}

read_experiment_data <- function(dataset){
  bp <- paste0("../datasets/", dataset)
  x  <- read.csv(file=paste0(bp, "_X.csv"), header = TRUE, row.names=1)
  ys <- data.matrix(
          read.csv(file=paste0(bp, "_Ys.csv"), header = TRUE, row.names=1)
        )
  splits <- generate_train_test_split(nrow(ys))
  
  x_learning  <- x[splits$train_indices, ] 
  x_test      <- x[splits$test_indices, ] 
  ys_learning <- ys[splits$train_indices, , drop = FALSE]
  ys_test     <- ys[splits$test_indices, , drop = FALSE]
