...
read_experiment_data <- function(dataset){
  x_learning <- data.matrix(
                  read.csv("../datasets/X_train.csv", header = TRUE, row.names = 1)
                )
  x_test     <- data.matrix(
                  read.csv("../datasets/X_test.csv", header = TRUE, row.names = 1)
                )
  ys_learning <- data.matrix(
                   read.csv(paste0("../datasets/", dataset, "_Ys_train.csv"),
                            header = TRUE, 
                            row.names = 1)
                 )
  ys_test     <- data.matrix(
                    read.csv(paste0("../datasets/", dataset, "_Ys_test.csv"),
                             header = TRUE, 
                             row.names = 1)
                 )
